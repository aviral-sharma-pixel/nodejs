    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#0a0e27' }}>
      ...
      <main style={{ marginLeft: '280px', flex: 1, padding: '2.5rem', overflow: 'auto', backgroundColor: '#0f1729' }}>
        ...
        <div style={{ marginBottom: '2.5rem' }}>
          <h1 style={{ fontSize: '2.25rem', margin: 0, fontWeight: '700', background: 'linear-gradient(to right, #fff, #a5f3fc)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Search Tickets 🔍</h1>
          <p style={{ margin: '0.5rem 0 0 0', opacity: 0.6, fontSize: '0.95rem' }}>Find tickets by ID or summary</p>
        </div>
        ...
        <form onSubmit={handleSearch} style={{ marginBottom: '2rem' }}>
          <div style={{ position: 'relative', display: 'flex', gap: '0.75rem' }}>
            <div style={{ position: 'relative', flex: 1 }}>
              <SearchIcon size={18} style={{ position: 'absolute', left: '1.25rem', top: '50%', transform: 'translateY(-50%)', opacity: 0.5 }} />
              <input 
                type="text" 
            ...
            <button
              ...
              style={{
                padding: '1rem 2rem',
                backgroundColor: searchQuery.trim() ? 'var(--primary)' : 'rgba(99, 102, 241, 0.5)',
                border: 'none',
                borderRadius: '10px',
                color: 'white',
                fontWeight: '600',
                ...
                alignItems: 'center',
                gap: '0.5rem',
                boxShadow: searchQuery.trim() ? '0 4px 14px 0 rgba(99, 102, 241, 0.39)' : 'none',
                transition: 'all 0.2s',
        ...
        {searchQuery && !loading && !error && tickets.length === 0 ? (
          <div style={{ padding: '4rem 2rem', textAlign: 'center', backgroundColor: 'rgba(25, 28, 50, 0.4)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.08)' }}>
            <h2 style={{ margin: 0, fontSize: '1.5rem' }}>No results found</h2>
            <p style={{ margin: '0.75rem 0 0 0', opacity: 0.6 }}>Try a different search term</p>
          </div>
        ) : searchQuery && tickets.length > 0 ? (
          <div>
            <p style={{ opacity: 0.6, fontSize: '0.875rem', marginBottom: '1.5rem', fontWeight: '500' }}>
              Found <span style={{ color: 'var(--primary)', fontWeight: '600' }}>{tickets.length}</span> ticket{tickets.length !== 1 ? 's' : ''}
            </p>
          ...
          <div style={{ padding: '4rem 2rem', textAlign: 'center', backgroundColor: 'rgba(25, 28, 50, 0.4)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.08)' }}>
            <h2 style={{ margin: 0, fontSize: '1.5rem' }}>Start searching</h2>
            <p style={{ margin: '0.75rem 0 0 0', opacity: 0.6 }}>Enter a ticket ID or summary above to search</p>
          </div>
        ) : null}
      </main>
